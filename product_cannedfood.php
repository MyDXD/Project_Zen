...
<head>
    ...
    <title>สินค้าอาหารกระป๋อง</title>
    <style>
        .product_container {
            display: flex;
            flex-wrap: wrap;
...
<body>
    ...
    ?>
    <h2 style="text-align: center;">สินค้าอาหารกระป๋อง</h2>
    <div class="product_container">

    <?php
        if (isset($_POST['search'])) {
            $search = isset($_POST['search']) ? $_POST['search'] : '';
            $sql = "SELECT * FROM `productdata` WHERE `product_type` = 'อาหารกระป๋อง' AND `product_name` LIKE '%$search%'";
        } else {
            $sql = "SELECT * FROM `productdata` WHERE `product_type` = 'อาหารกระป๋อง'";
        }
        $result = $db->query($sql);
