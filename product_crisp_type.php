...
<body>
    ...
    ?>
    <h2 style="text-align: center;">สินค้าอบกรอบ</h2>
    <div class="product_container">
        
        <?php
        if (isset($_POST['search'])) {
            $search = isset($_POST['search']) ? $_POST['search'] : '';
            $sql = "SELECT * FROM `productdata` WHERE `product_type` = 'อบกรอบ' AND `product_name` LIKE '%$search%'";
        } else {
            $sql = "SELECT * FROM `productdata` WHERE `product_type` = 'อบกรอบ'";
        }
        $result = $db->query($sql);
